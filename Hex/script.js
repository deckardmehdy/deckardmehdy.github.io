...
class minMaxTree {
	...
	getNextMove ( player ) {
		this.traverseTree( -1, -1, true, player, 0 );
	...
	traverseTree ( row, col, maximizingPlayer, player, depth ) {
		...
		if ( maximizingPlayer ){
			...
			for ( var i = 0; i < totalRows; i++ ){
				for ( var j = 0; j < totalCols; j++ ){
					if ( !this.visited[i][j] ){
						//console.log( "-------------" );
						//console.log( "Child being visited = [ " + i + ", " + j + " ]" );
						this.makeMove( i, j, player );
						var newVal = this.traverseTree( i, j, false, player, depth + 1 );
						if ( row == -1 & col == -1 ){
							//console.log( "Val at child node = " + newVal );
						}
						if ( val < newVal ){
							val = newVal;
							if ( depth == 0 ){ 
								...
								console.log( "Next move being updated -> " + newVal + " -> [ " + i + " , " + j + " ]");
							} // root node
						}
			...
			for ( var i = 0; i < totalRows; i++ ){
				for ( var j = 0; j < totalCols; j++ ){
					if ( !this.visited[i][j] ){
						...
						val = Math.min( val, this.traverseTree( i, j, true, player, depth + 1 ) );
						this.releaseMove( i, j );
					}
				}
			}
...
function resetGame (){
	...
	$( "#playMove" ).hide(speed);
	$( ".key" ).hide("fast");
	$( "#keyContainer" ).hide("fast");

	$( "#welcomeMessage" ).show(speed);
...
$( "#p2btn" ).click(function(){
	...
	startGame();
	//console.log( "Hex board after creating game: " );
	//console.log( hexBoard );
